...
function [AIC_RW, BIC_RW, AIC_PH, BIC_PH, alph_est, alph0_est, gamm_est, ...
    cost_RW, cost_PH] = pesd_fits_no_scaling(data,con, ntrials, Nsub,starttrial)
...
P = P(starttrial:ntrials,:);
R = R(starttrial:ntrials,:);
...
cost_PH = cost;
AIC_PH = 2*(k + cost);
BIC_PH = k*log(M) + 2*cost;fprintf('alph0_est = %4.1f - gamm_est    = %4.2f - sigma_N_est = %4.1f - AIC_PH = %4.1f\n',alph0_est,gamm_est,sqrt(sigma_N_est),AIC_PH)
...
for c = 1:Nc
    ...
    mu_n(n1,c)    = mu_pri + alph_PH(n1,c)*PE;
