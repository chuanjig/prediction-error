function [AIC_RW, BIC_RW, AIC_PH, BIC_PH, alph_est, alph0_est, gamm_est, cost_RW, ...
    cost_PH, ns_est_unsign, ns_est_sign] = pesd_fits_est_scaling_2param(data,con, ntrials, Nsub)

SD = repmat([5 10 15 5 10 15],1,Nsub); EV = repmat([35 35 35 65 65 65],1,Nsub); % Joost aanpassen
...
warning off % 
[F_fit, cost] = fmincon(cf,F,[],[],[],[],lb,ub,[],opt,x,aux,data); warning on % #ok
...
F     = [alph0 gamm sN_PH ns_unsign ns_sign];                           % Initialize
lb    = [    0    0     1 0 0];                           % Lower bounds
ub    = [    1    1    15 1 1];                           % Upper bounds (for ns, original is 1)
opt = optimset('Display','off','TolX',1e-6,'TolFun',1e-9,'algorithm','sqp','FunValCheck','on');
warning off %
[F_fit, cost] = fmincon(cf,F,[],[],[],[],lb,ub,[],opt,x,aux,data); warning on % #ok
cf(F_fit,x,aux,data); % check for zero likelihood
...
ns_unsign= P(4);                % initial normscale PE
ns_sign  = P(5);                % initial normscale PE
l        = nan(size(x));        % Likelihood
s2       = nan(size(x));        % Variance of likelihood
mu_n     = nan(size(x));        % Posterior mean
...
for c = 1:Nc
    ...
    % Init
    n1 = keep(1);
    subsd_sign    = 1-ns_sign + ns_sign * SD(c);
    subsd_unsign    = 1-ns_unsign + ns_unsign * SD(c);
    PE = x(n1,c) - mu_pri;      % Prediction error
    alph_PH(n1,c) = gamm*abs(((PE/100)/subsd_unsign)) + (1-gamm)*(alph_0);
    mu_n(n1,c)    = mu_pri + alph_PH(n1,c)*(PE/subsd_sign);
    ...
    for ind = 2:Ni
        ...
        % Posterior estimate
        alph_PH(n,c) = gamm*abs(((PE/100)/subsd_unsign)) + (1-gamm)*(alph_PH(n_1,c));
