function [AIC_RW, BIC_RW, AIC_PH, BIC_PH, alph_est, alph0_est, gamm_est, ...
    cost_RW, cost_PH, ns_est]= pesd_fits_est_scaling(data,con, ntrials, Nsub)
...
F     = [alph0 gamm sN_PH ns];                           % Initialize
lb    = [    0    0     1 0];                           % Lower bounds
ub    = [    1    1    15 1];                           % Upper bounds (for ns, original is 1)
opt = optimset('Display','off','TolX',1e-6,'TolFun',1e-9,'algorithm','sqp','FunValCheck','on');
warning off 
